...
La struttura di tale paradigma si divide in tre parti fondamentali: il \textit{Dispatcher}, lo \textit{Store} e la \textit{View}. Opzionalmente esiste un altro componente chiamato \textit{Action Creator} che si occupa di emanare le varie azioni disponibili.
...
Flux considera nella sua architettura quattro elementi fondamentali: Le \textit{Actions}, il \textit{Dispatcher}, gli \textit{Stores} e le \textit{View}.

\subsubsection*{Action}
Le Actions sono la moneta di scambio dell'architettura. Queste consistono generalmente in un oggetto composto da due elementi:
...
\noindent
Tutte gli eventi che modificano lo stato esistenti nell'applicazione devono essere descritti sotto forma di Action. In Flux qualsiasi cosa non sia un'azione non può verificarsi.

\subsubsection*{Dispatcher}
Il Dispatcher nell'architettura Flux è il componente che ha il compito di distribuire le azioni agli Stores. È sostanzialmente un registro di callback all'interno degli Stores, e non ha una vera e propria logica sua fungendo quindi solo da trasmettitore. La sua particolarità è che qualsiasi azione riceva, viene trasmessa poi ad ogni callback registrata lasciando a quest'ultima il compito di analizzare l'azione e decidere se effettuare cambiamenti o meno.
